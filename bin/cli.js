...
require("yargs")
  .config(config)
  .config("config", "Path to JavaScript config file", customConfigPath =>
  ...
  .command(
    ...
    "The default command",
    {
      ...
      url: {
        alias: "u",
        describe:
          'URL to open. When using the "serve" option, URL should be root relative.'
      },
      file: {
        alias: "f",
      ...
      viewports: {
        alias: "v",
        describe:
          "Set of named viewport sizes for symbols, e.g. --viewports.Desktop=1024x768 --viewports.Mobile=320x568"
      },
      debug: {
        alias: "d",
      ...
      "symbol-middleware": {
        describe:
          "Path to symbol middleware to run when looping over sketch layers"
      },
      "puppeteer-args": {
        type: "string",
        describe:
          'Set of command line arguments to be provided to the Chromium instance via Puppeteer, e.g. --puppeteer-args="--no-sandbox --disable-setuid-sandbox"'
      },
      "puppeteer-executable-path": {
        type: "string",
        describe:
          "Path to a Chromium executable to use instead of the one downloaded by Puppeteer."
      },
      "puppeteer-wait-until": {
        type: "string",
        describe:
          "The Puppeteer navigation event to use before considering the page loaded.",
    ...
    async argv => {
      try {
        ...
        try {
          const url = argv.file
            ? `file://${path.join(process.cwd(), argv.file)}`
            : argv.url;
          const symbolsUrl = argv.serve
            ? urlJoin(`http://localhost:${String(port)}`, argv.url || "/")
            : url;
          const debug = argv.debug;

          const symbolLayerMiddleware = resolveMiddleware(
            argv.symbolLayerMiddleware
          );
          const symbolMiddleware = resolveMiddleware(argv.symbolMiddleware);
          const symbolInstanceMiddleware = resolveMiddleware(
            argv.symbolInstanceMiddleware
          );
          ...
          try {
            ...
            const hoverIds = (await cdp.send("DOM.querySelectorAll", {
              nodeId: docNodeId,
              selector: '[data-sketch-hover="true"] > *'
            })).nodeIds;

            await cdp.send("CSS.enable");
            for (const nodeId of hoverIds) {
            ...
            const focusIds = (await cdp.send("DOM.querySelectorAll", {
              nodeId: docNodeId,
              selector: '[data-sketch-focus="true"] > *'
            })).nodeIds;
              ...
              if (viewports.hasOwnProperty(viewportName)) {
                ...
                await page.evaluate(
                  `generateAlmostSketch.snapshotSymbols({ suffix: "${
                    hasViewports ? `/${viewportName}` : ""
                  }", symbolLayerMiddleware: ${symbolLayerMiddleware}, symbolMiddleware: ${symbolMiddleware}, symbolInstanceMiddleware: ${symbolInstanceMiddleware}  })`
                );
              }
            }
